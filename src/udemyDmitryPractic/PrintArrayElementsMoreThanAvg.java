...
class PrintArrayElementsMoreThanAvg {
    public static void main(String[] args) {
        ...
        var min = Arrays.stream(moreThanAverage).min().orElseThrow();
        var max = Arrays.stream(moreThanAverage).max().orElseThrow();
    ...
    private static int calculateAverage(int[] array) {
        return (int) Arrays.stream(array).average().orElseThrow();
    }
